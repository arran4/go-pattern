...
- **[Gallery](GALLERY.md)**: A visual guide to all available base patterns with parameters and CLI usage.
- **[Recipes](RECIPES.md)**: A cookbook of complex textures (grass, wood, stone, etc.) created by composing patterns.
...
func ExampleNewGrass() {
	...
	baseNoise := NewNoise(SetNoiseAlgorithm(&PerlinNoise{Seed: 500, Frequency: 0.02, Octaves: 4}))

	// 2. Create a high-frequency noise layer
	detailNoise := NewNoise(SetNoiseAlgorithm(&PerlinNoise{Seed: 600, Frequency: 0.2, Octaves: 2}))
	...
	grass := NewColorMap(blended,
		ColorStop{0.0, color.RGBA{10, 40, 10, 255}},
		ColorStop{1.0, color.RGBA{140, 220, 100, 255}},
	)
    // ...
}
```
