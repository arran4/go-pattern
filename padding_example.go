...
func BootstrapPaddingReferences() (map[string]func(image.Rectangle) image.Image, []string) {
	...
	}, []string{"Color", "Bounded", "Centered", "TopLeft", "BottomRight", "Right"}
}
...
func NewDemoPaddingTopLeft(ops ...func(any)) image.Image {
	...
	// Align Top-Left with 20px padding
	return NewAligned(gopher, 150, 150, 0.0, 0.0, image.NewUniform(color.RGBA{220, 220, 220, 255}), 20)
}

func NewDemoPaddingBottomRight(ops ...func(any)) image.Image {
	gopher := NewScale(NewGopher(), ScaleToRatio(0.2))
	// Align Bottom-Right with 10px padding
	return NewAligned(gopher, 150, 150, 1.0, 1.0, image.NewUniform(color.RGBA{220, 220, 220, 255}), 10)
}

func NewDemoPaddingRight(ops ...func(any)) image.Image {
	gopher := NewScale(NewGopher(), ScaleToRatio(0.2))
	// Align Right (Vertically Centered) with 30px horizontal padding
	return NewAligned(gopher, 150, 150, 1.0, 0.5, image.NewUniform(color.RGBA{220, 220, 220, 255}), 30, 0)
}
