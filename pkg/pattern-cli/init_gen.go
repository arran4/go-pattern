...
func RegisterGeneratedCommands(fm dsl.FuncMap) {
	...
	fm["clamp"] = func(args []string, input image.Image) (image.Image, error) {
		if len(args) < 1 {
			return nil, fmt.Errorf("clamp requires 1 arguments")
		}
		return nil, fmt.Errorf("command clamp has unsupported argument types")
	}
	...
	fm["remap"] = func(args []string, input image.Image) (image.Image, error) {
		if len(args) < 0 {
			return nil, fmt.Errorf("remap requires 0 arguments")
		}
		if input == nil {
			return nil, fmt.Errorf("remap requires an input image")
		}
		return pattern.NewRemap(input), nil
	}
